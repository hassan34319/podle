...
import LeaveReviewMain from "./LeaveReviewMain";
...
function UserReviews({ reviews, rating,title }: Props) {
  ...
  return (
    <div className="h-max pb-[5vh] pt-[5vh] px-[4%]">
      ...
      <div className="flex md:flex-row flex-col justify-between">
        ...
        {/* Rating */}
        {/* Review Button */}
        <LeaveReviewMain businessName={title}/>
      </div>
        ...
        {reviews.length > 0 ? (
          reviews.map((user, index) => (
            <ReviewCard
              ...
              date={user.date}
              reviewRating={user.reviewRating}
            />
          ))
        ) : (
